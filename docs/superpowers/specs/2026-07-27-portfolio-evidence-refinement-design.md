...
| BOOT-07 | 실제 Alignment 이미지 사용 | Notion 기록의 실제 코드·PC·BTV·Breakpoint 값을 로컬 이미지로 구성하고 만료형 URL을 노출하지 않는다. 원본 캡처 바이너리는 커넥터에서 직접 내보낼 수 없어 검증 결과를 PARTIAL로 기록한다. |
...
| CREDENTIAL-01 | 확대 이미지 생성 | 마스킹 PDF 첫 페이지를 200 DPI PNG로 렌더링해 확대 보기 자산으로 사용한다. |
