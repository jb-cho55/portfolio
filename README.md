...
2. 백업 번들 해시를 확인하고 `git ls-remote origin refs/heads/main` 결과가 위의 기존 원격 `main` SHA와 같은지 확인합니다.
3. Pages의 **Build and deployment** source를 **GitHub Actions**로 변경합니다. CLI에서는 `gh api --method PUT repos/jb-cho55/portfolio/pages -f build_type=workflow`을 사용하거나, GitHub UI의 `Settings → Pages → Build and deployment → Source → GitHub Actions`에서 변경할 수 있습니다.
4. 원격 SHA가 일치할 때만 다음 guarded push를 한 번 실행합니다.

   ```powershell
   git push --force-with-lease=refs/heads/main:661b9d5186028318a1f181f4f7b0e8a822f1da63 origin HEAD:refs/heads/main
   ```

5. push로 생성된 실행 ID를 확인한 뒤 완료될 때까지 기다리고 실패 로그를 검사합니다.
...
6. workflow가 성공한 뒤 `gh api repos/jb-cho55/portfolio/pages`의 `build_type`과 배포 URL을 확인하고, 공개 홈과 세 사례 경로가 HTTP 200을 반환하는지 확인합니다. 워크플로 성공과 실제 배포 응답을 모두 확인하기 전에는 게시가 완료됐다고 판단하지 않습니다.
